...
// --- Clip-path helpers (return CSS clip-path strings for any element) ---
...
export type ClipWipeDirection = "left" | "right" | "up" | "down";
...
function clipCircle(progress: number, opts?: CircleRevealOptions): string {
  const t = clamp01(progress);
  const cx = (opts?.cx ?? 0.5) * 100;
  const cy = (opts?.cy ?? 0.5) * 100;
  const maxR = sqrt(max(cx, 100 - cx) ** 2 + max(cy, 100 - cy) ** 2);
  const radius = r3(t * maxR);
  return `circle(${radius}% at ${r3(cx)}% ${r3(cy)}%)`;
}

function clipWipe(progress: number, direction: ClipWipeDirection = "right"): string {
  const t = clamp01(progress);
  const p = r3(t * 100);
  switch (direction) {
    case "right":
...
function clipInset(progress: number, opts?: InsetRevealOptions): string {
  const t = clamp01(progress);
...
function clipIris(progress: number, sides: number = 6): string {
  const t = clamp01(progress);
  const radius = t * 150;
  const points: string[] = [];
...
/** Clip-path reveal helpers — use on any element via `clip-path` style */
