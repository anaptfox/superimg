...
```bash
node ./packages/superimg/dist/cli.js new <VIDEO_NAME> [options]
```
   Examples:
   ```bash
   # Interactive — prompts for type, tailwind, language
   node ./packages/superimg/dist/cli.js new my-promo

   # Single scene with Tailwind, non-interactive
   node ./packages/superimg/dist/cli.js new my-promo --tailwind -y

   # Multi-scene composition
   node ./packages/superimg/dist/cli.js new my-promo --compose
   ```
...
```bash
node ./packages/superimg/dist/cli.js dev <VIDEO_NAME>
```
