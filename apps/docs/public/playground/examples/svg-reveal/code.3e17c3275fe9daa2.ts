...
// Demonstrates: std.reveal.clip for circle, wipe, inset, and iris clip-path reveals
...
export default define({
  ...
  render(ctx) {
    ...
    const reveals = [
      { name: "CIRCLE", color: "#667eea", fn: (p: number) => std.reveal.clip.circle(p) },
      { name: "WIPE", color: "#f093fb", fn: (p: number) => std.reveal.clip.wipe(p, "right") },
      { name: "INSET", color: "#4fd1c5", fn: (p: number) => std.reveal.clip.inset(p) },
      { name: "IRIS", color: "#f6ad55", fn: (p: number) => std.reveal.clip.iris(p, 6) },
    ];
    ...
    // Stagger the reveals (ms API + 500ms cascade cap)
    ...
    // Overall fade
    const masterOpacity = std.interpolate(timeline.progress, [0, 0.05, 0.9, 1], [0, 1, 1, 0]);
