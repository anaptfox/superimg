...
export default define<EventAgendaVideoData>({
  sample: {
  ...
  render(ctx: RenderContext<EventAgendaVideoData>) {
    ...
    const headerProgress = std.interpolate(timeline.progress, [0, 0.15], [0, 1], "easeOutBack");
    ...
    sessions.forEach((session: EventSession, i: number) => {
      ...
      const sessionProgress = std.interpolate(timeline.progress, [start, end], [0, 1], "easeOutCubic");

      if (sessionProgress > 0) {
        maxRevealedIndex = i;
        const holdProgress = std.interpolate(timeline.progress, [end, 1], [0, 1]);
    ...
    const lineProgress = std.interpolate(timeline.progress, [0.15, 0.9], [0, lineHeight], "easeOutCubic");

    let dotsHtml = "";
    sessions.forEach((_: EventSession, i: number) => {
      const start = 0.15 + i * sessionDuration;
      const end = start + Math.min(sessionDuration, 0.1);
      const dotProgress = std.interpolate(timeline.progress, [start, end], [0, 1], "easeOutBack");
