...
export default define({
  ...
  render(ctx) {
    ...
    // Director phases: 1.5s enter | 1.5s hold | 1.0s exit
    const t = ctx.director({ enter: "1.5s", hold: "1.5s", exit: "1.0s" });

    const lineAnim     = t.motion({ at: "0%",    for: "80%",  easing: "easeOutCubic" });
    const titleAnim    = t.motion({ at: "15%", for: "85%", y: 40, easing: "easeOutCubic" });
    const subtitleAnim = t.motion({ at: "30%",  for: "85%", y: 30, easing: "easeOutCubic" });
