...
export default define({
  ...
  render(ctx) {
    ...
    const t = ctx.director({ enter: "2.5s", hold: "2.5s" });
    const morphInP = t.in("enter");
    const morphOutP = t.in("hold");
    ...
    const rotation = timeline.progress * 45;

    // Overall fade
    const opacity = std.interpolate(timeline.progress, [0, 0.08, 0.92, 1], [0, 1, 1, 0]);

    // Label
    const labelOpacity = std.interpolate(timeline.progress, [0.05, 0.15, 0.9, 1], [0, 0.7, 0.7, 0]);
