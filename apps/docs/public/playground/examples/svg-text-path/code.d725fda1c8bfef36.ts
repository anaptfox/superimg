...
export default define({
  ...
  render(ctx) {
    ...
    const breathe = 1 + Math.sin(timeline.progress * Math.PI * 2) * 0.2;
    ...
    const offset1 = std.interpolate(timeline.progress, [0, 1], [-10, 60], "linear");
    const offset2 = std.interpolate(timeline.progress, [0, 1], [70, 10], "linear");

    // Fade in/out
    const opacity = std.interpolate(timeline.progress, [0, 0.1, 0.85, 1], [0, 1, 1, 0]);
    ...
    const glowOpacity = 0.15 + Math.sin(timeline.progress * Math.PI * 3) * 0.1;
