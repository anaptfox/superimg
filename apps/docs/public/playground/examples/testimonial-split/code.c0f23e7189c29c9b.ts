...
export default define({
  ...
  render(ctx) {
    ...
    const wipeEased = t.in("intro", { at: "0%", duration: "70%"});
    const wipe = std.reveal.wipe({ progress: wipeEased, direction: "diagonal", color: brandColor });

    // Header + photo animations
    const headerAnim = t.motion({ at: "60%", for: "40%", exit: false });
    const photoAnim = t.motion({ at: "70%", for: "50%", scale: 0.2 });
    const footerAnim = t.motion({ at: "80%", for: "50%"});
    ...
    const renderQuoteChunk = () => {
      ...
      const chunkIndex = Math.floor(t.in("content") * quoteChunks.length);
      const chunk = quoteChunks[chunkIndex];
      const localP = (t.in("content") * quoteChunks.length) % 1;
