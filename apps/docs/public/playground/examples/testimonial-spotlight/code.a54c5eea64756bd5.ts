...
export default define({
  ...
  render(ctx) {
    ...
    const photoGrow = t.in("intro", { at: "0%", duration: "50%"});
    ...
    const shiftProgress = t.in("intro", { at: "50%", duration: "50%"});
    ...
    const renderSentences = () => {
      return (quoteSentences as string[]).map((sentence: string, index: number) => {
        ...
        const sP = t.in("content", { at, duration: dur });
