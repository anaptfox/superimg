...
export default define({
  ...
  render(ctx) {
    ...
    const t = ctx.director({ enter: "3.0s", hold: "1.0s", exit: "1.0s" });
    ...
    // Subtitle text
    const textOpacity = std.interpolate(timeline.progress, [0.3, 0.5, 0.85, 1], [0, 1, 1, 0]);
    const textY = std.interpolate(std.clamp01((timeline.progress - 0.3) / 0.2), [0, 1], [20, 0], "easeOutCubic");
