...
export default define<HttpTraceData>({
  sample: {
    method: "POST",
    endpoint: "/v1/users",
  ...
  render(ctx: RenderContext<HttpTraceData>) {
    ...
    const layoutP = t.in("layout");
    const stageOp = 1 - layoutP;
    const stageScale = std.spring(1.04, 1, t.in("stage"), "gentle");
    ...
    const statusP = t.in("status");
    const statusScale = std.spring(0.7, 1, statusP, "playful");
    const statusOp = std.interpolate(statusP, [0, 1], [0, 1], "easeOutCubic");
    const settlePulse = t.active === "settle" ? 1 + Math.sin(t.in("settle") * Math.PI) * 0.04 : 1;

    const calloutP = t.in("callout");
    const calloutScale = std.spring(0.7, 1, calloutP, "snappy");
    ...
    const reqStream = reqBodyPretty
      ? streamCode(std, reqBodyPretty, t.in("request"), {
          lang: "json",
          theme: palette.shiki,
          variance: 0.7,
    ...
    const resStream = resBodyPretty
      ? streamCode(std, resBodyPretty, t.in("response"), {
          lang: "json",
          theme: palette.shiki,
          variance: 0.7,
          time: timeline.seconds,
