...
export default define<TimelineVideoData>({
  sample: {
  ...
  render(ctx: RenderContext<TimelineVideoData>) {
    ...
    const titleProgress = std.interpolate(timeline.progress, [0, 0.1], [0, 1], themeEasing);
    ...
    const displayIndex = activeNodeIndex === -1 ? (timeline.progress > 0.5 ? totalEvents - 1 : 0) : activeNodeIndex;

    const fillPercent = timeline.progress < ENTRANCE_END
      ? 0
      : Math.min(100, ((timeline.progress - ENTRANCE_END) / progressRemaining) * 100);
