...
export default define({
  ...
  render(ctx) {
    ...
    // director phases: intro (2s) → features (4s) → outro (2s)
    const t = ctx.director({ intro: "2.0s", features: "4.0s", outro: "2.0s" });

    // Phase-specific rendering based on which is active
    if (t.active === "intro") {
      return renderIntro(std, t.in("intro"), title);
    } else if (t.active === "features") {
      return renderFeatures(std, t, features);
    } else if (t.active === "outro") {
      return renderOutro(std, t.in("outro"));
    }
