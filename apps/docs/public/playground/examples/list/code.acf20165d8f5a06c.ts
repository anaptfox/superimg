...
export default define<ListVideoData>({
  ...
  render(ctx: RenderContext<ListVideoData>) {
    ...
    const stk = std.stack(orderedItems, { during: t.in("items"), weights });
    ...
    const fadeOutProgress = std.interpolate(t.in("outro"), [0, 1], [0, 1], "easeInCubic");
