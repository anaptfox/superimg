...
Use `std.score()` and its `t.tween()` helper when motion belongs to named scene phases like enter, hold, and exit. Use `std.interpolate()` when you already have a progress value and need custom ranges, multi-keyframes, loops, or non-phased math. Note: `tween` lives on the score object — there is no `std.tween()` at the top level. Always reach it via `const t = std.score(); t.tween(...)`.
