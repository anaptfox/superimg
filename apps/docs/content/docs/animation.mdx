...
Use `ctx.director()` and its `d.tween()` helper when motion belongs to named scene phases like enter, hold, and exit. Use `std.interpolate()` when you already have a progress value and need custom ranges, multi-keyframes, loops, or non-phased math. Note: `tween` lives on the director — there is no `std.tween()` at the top level. Always reach it via `const d = ctx.director(); d.tween(...)`.
...
## Sequenced Animations (ctx.director)

`ctx.director()` is the primary orchestration primitive for layouts. It breaks your scene into logical phases (e.g. intro, content, outro) and gives you motions scoped to those phases.
...
render(ctx) {
  ...
  const t = ctx.director({ enter: 0.25, hold: 0.5, exit: 0.25 });
...
`ctx.director()` handles the exit-math automatically, eliminating the need to manually multiply by `(1 - exitProgress)`.
...
```typescript
// Inside director motion, use spring easing as a string
const t = ctx.director();
const pop = t.motion({ scale: 0.5, easing: "spring(200,8)" });

// Standalone spring interpolation
const scale = std.spring(0.8, 1, ctx.timeline, { stiffness: 200, damping: 8 });
```
...
const opacity = std.interpolate(ctx.timeline, [0, 0.2, 0.8, 1], [0, 1, 1, 0]);

// Multi-stop position
const x = std.interpolate(ctx.timeline, [0, 0.5, 1], [0, 500, 300]);
```

For color transitions:

```typescript
const bg = std.interpolateColor(ctx.timeline,   [0, 0.3, 0.7, 1],
...
render(ctx) {
  ...
  const staggered = std.stagger(items, timeline, {
    duration: 0.3,
    easing: "easeOutCubic",
  });
...
- [Timing With Director And Cues](/docs/timing) — Director phases, cue timestamps, and how to combine them
- [How It Works](/docs/how-it-works) — Understand the rendering pipeline
- [CLI Reference](/docs/cli) — Commands for rendering to MP4, GIF, and stills
