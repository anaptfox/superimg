...
```bash
npx superimg render template --data '{"title": "Dynamic Video", "subtitle": "Rendered from CLI"}'
```

Output is written to `output/template.mp4` next to your template file. SuperImg renders every frame headlessly and encodes it into a high-quality MP4.
...
npx superimg render og-card.media.ts
```
