...
SuperImg ships a built-in `std` library so you are not writing animation math from scratch. Use `std.score()` to orchestrate scene phases, `std.interpolate()` for lower-level value mapping, and `std.color` to work with gradients and palettes.
