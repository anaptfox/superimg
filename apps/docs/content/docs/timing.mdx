# Timing With Director And Cues

SuperImg has two timing primitives for most template work:

- `ctx.director()` choreographs scene-local phases from `timeline.progress`.
- `std.cue.*` follows absolute timestamps from transcripts, markers, audio, or scripts.

Use `ctx.director()` when the scene owns the timing. Use `cue` when external data owns the timing.

## Enter, Hold, Exit

`ctx.director()` defaults to `enter`, `hold`, and `exit` phases. It gives you ready-to-use motion styles and handles the fade-out math automatically.
...
render(ctx) {
  ...
  const commandProgress = t.in("terminal");
...
`t.active` tells you which phase is currently running, and `t.in("phase")` gives progress scoped to that phase.

## Nested Clips

`director.clip()` opens nested timing windows — like Remotion sequences scoped to a parent phase or explicit time range. Each clip gets its own local `director()`:
...
render(ctx) {
  ...
  if (hook.active) {
    ...
    return `<div style="${std.css({ width, height }, std.css.center())}">
      <h1 style="${card.style}">${data.hook}</h1>
    </div>`;
  }

  // ... demo and outro clips
}
```
...
Use `ctx.director()` for the frame and `cue` for audio-synced content inside it.

```typescript
render(ctx) {
  const { std, timeline, data } = ctx;
  const scene = ctx.director({ enter: 0.12, captions: 0.76, exit: 0.12 });
