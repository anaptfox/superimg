...
// Demonstrates: std.layers(), std.score(), std.reveal.*, SVG iPhone + in-app UI, responsive outputs
...
export default defineScene({
  ...
  config: {
    ...
    audio: { src: "../../_assets/lofi-bg.mp3", volume: 0.45, fadeIn: 0.5, fadeOut: 2, loop: true },
    outputs: {
  ...
  render(ctx) {
    ...
    const hookLocalLive = t.within("hook");
    const featuresLocalLive = t.within("features");
    const ctaLocalLive = t.within("cta");

    function phaseEnter(local: number, at: number, forDur: number, y: number) {
      const raw = std.clamp01((local - at) / forDur);
      const enter = std.interpolate(raw, [0, 1], [0, 1], "easeOutCubic");
      return `opacity:${enter};transform:translateY(${(1 - enter) * y}px)`;
    }

    function phaseEnterScale(local: number, at: number, forDur: number, scaleDelta: number) {
      const raw = std.clamp01((local - at) / forDur);
      const enter = std.interpolate(raw, [0, 1], [0, 1], "easeOutCubic");
      const scale = 1 - (1 - enter) * scaleDelta;
      return `opacity:${enter};transform:scale(${scale})`;
    }
    ...
    function buildHookContent(hookLocal: number) {
      const headlineStyle =
        hookLocal >= 1 ? STEADY_MOTION : phaseEnter(hookLocal, 0.08, 0.38, 32);
      const subStyle =
        hookLocal >= 1 ? STEADY_MOTION : phaseEnter(hookLocal, 0.18, 0.36, 22);
    ...
    function buildHookShot(hookLocal: number, introWipe?: ReturnType<typeof std.reveal.wipe>) {
      const phoneMotion =
        hookLocal >= 1 ? STEADY_MOTION : phaseEnterScale(hookLocal, 0.12, 0.48, 0.06);
      const layers = [
        L.bg(APP_BG),
        L.tint(isPortrait ? SCRIM_CENTER : SCRIM_LEFT),
        L.content(buildHookContent(hookLocal), { safe: "broadcast", inset: textInset() }),
    ...
    function buildFeaturesContent(featuresLocal: number) {
      ...
      const metricRaw = std.clamp01((featuresLocal - 0.48) / 0.38);
      const metricCount = Math.floor(
        std.interpolate(metricRaw, [0, 1], [0, metric.value], "easeOutCubic"),
      );
      const metricStyle =
        featuresLocal >= 1 ? STEADY_MOTION : phaseEnterScale(featuresLocal, 0.48, 0.38, 0.08);
      const badgeStyle =
        featuresLocal >= 1 ? STEADY_MOTION : phaseEnter(featuresLocal, 0.06, 0.32, -16);
    ...
    function buildHookPanel(hookLocal: number) {
      return L.render(
        L.tint(isPortrait ? SCRIM_CENTER : SCRIM_LEFT),
        L.content(buildHookContent(hookLocal), { safe: "broadcast", inset: textInset() }),
      );
    ...
    function buildCtaContent(ctaLocal: number, live = false) {
      const kickerStyle = live
        ? t.motion({ during: "cta", at: 0.04, for: 0.4, y: 14 }).style
        : phaseEnter(ctaLocal, 0.04, 0.4, 14);
      const headlineStyle = live
        ? t.motion({ during: "cta", at: 0.1, for: 0.55, y: 28 }).style
        : phaseEnter(ctaLocal, 0.1, 0.55, 28);
      const tagStyle = live
        ? t.motion({ during: "cta", at: 0.18, for: 0.45, y: 18 }).style
        : phaseEnter(ctaLocal, 0.18, 0.45, 18);
      const btnStyle = live
        ? t.motion({ during: "cta", at: 0.28, for: 0.5, scale: 0.1 }).style
        : phaseEnterScale(ctaLocal, 0.28, 0.5, 0.1);
      ...
      const lowerMotion = live
        ? t.motion({ during: "cta", at: 0.42, for: 0.45, y: 32 })
        : undefined;
      const lowerStyle = live ? "" : phaseEnter(ctaLocal, 0.42, 0.45, 32);

      const ctaLowerThird = `
        <div class="url-pill" style="${std.css({ display: "flex", alignItems: "center", gap: 12 })}; ${lowerStyle}">
    ...
    function buildCtaShot(ctaLocal: number, live = false) {
      const { ctaCenterHtml, ctaLowerThird, lowerMotion } = buildCtaContent(ctaLocal, live);
      const streak = Math.floor(std.interpolate(ctaLocal, [0.2, 0.52], [0, 12], "easeOutCubic"));
      const phoneMotion = live
        ? t.motion({ during: "cta", at: 0.08, for: 0.55, scale: 0.06 }).style
        : phaseEnterScale(ctaLocal, 0.08, 0.55, 0.06);
    ...
    if (t.inSpan("9.8s", "10.8s")) {
      ...
      const handoffCtaLocal = std.reveal.handoffLocal(featuresToCtaP);
      return std.reveal.crossfade({
        from: buildFeaturesShot(1),
        to: buildCtaShot(handoffCtaLocal),
    ...
    if (t.active === "hook") {
      ...
      return buildHookShot(hookLocalLive, introWipe);
    }
    ...
    return buildCtaShot(ctaLocalLive, true);
  },
});