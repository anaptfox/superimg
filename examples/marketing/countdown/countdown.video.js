...
// Demonstrates: std.css(), std.css.center(), std.spring(), std.interpolate(), color utilities
...
function generateParticles(std, time, count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const size = 4 + (i % 3) * 2;
    const opacity = std.interpolate((i % 5, [0, 1], [0.3, 0.7]) / 5);
...
export default defineScene({
  ...
  render(ctx) {
    ...
    const pulse = std.spring(1, 1.3, fraction, { stiffness: 300, damping: 8 });
    const numberOpacity = std.interpolate(fraction, [0, 1], [0.5, 1]);
    const ringProgress = std.interpolate(sceneProgress, [0, 0.1, 0.9, 1], [0, 0, 360, 360]);
    const bgHue = std.interpolate(sceneProgress, [0, 1], [20, 50]);
