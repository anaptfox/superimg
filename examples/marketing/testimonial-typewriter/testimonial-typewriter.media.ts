...
export default define({
  ...
  render(ctx) {
    ...
    const headerAnim = t.motion({ at: "20%", for: "80%"});

    // Photo + attribution (starts after header)
    const photoAnim = t.motion({ at: "40%", for: "80%", x: 100 });
    const attrAnim = t.motion({ at: "50%", for: "60%", exit: false });
    ...
    return `
      <div style="${std.css({ width, height, position: "relative", overflow: "hidden", background: "#000", padding })}">
        ...
        <!-- Attribution bar -->
        <div style="${std.css({ position: "absolute", bottom: padding + borderWidth, left: padding + borderWidth, right: padding + borderWidth, height: 56, background: brandColor, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", clipPath: `inset(0 ${100 - attrAnim.enter * 100}% 0 0)`, opacity: attrAnim.opacity * (1 - t.in("exit")) })}">
