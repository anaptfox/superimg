...
export default define<TweetVideoData>({
  ...
  render(ctx: RenderContext<TweetVideoData>) {
    ...
    const authorProgress = std.interpolate(timeline.progress, [TIMING.authorFadeIn.start, TIMING.authorFadeIn.end], [0, 1], "easeOutCubic");
    const textProgress = hasText ? std.interpolate(timeline.progress, [TIMING.textReveal.start, TIMING.textReveal.end], [0, 1]) : 0;
    const imagesProgress = std.interpolate(timeline.progress, [TIMING.imagesFadeIn.start, TIMING.imagesFadeIn.end], [0, 1], "easeOutCubic");
    const metricsProgress = std.interpolate(timeline.progress, [TIMING.metricsCountUp.start, TIMING.metricsCountUp.end], [0, 1], "easeOutCubic");
