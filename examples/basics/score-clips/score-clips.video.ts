...
export default defineScene({
  sample: {
    hook: "Ship faster.",
    demo: "Score clips, not timelines.",
    cta: "superimg.dev",
    accent: "#667eea",
  },
  ...
  render(ctx) {
    ...
    const t = std.score({ hook: "2s", demo: "3s", outro: "1s" });

    const hook = t.clip({ during: "hook" });
    if (hook.active) {
      const local = hook.score();
    ...
    if (demo.active) {
      const local = demo.score();
      const title = local.motion({ y: 24 });
      const highlight = feature.active
        ? feature.score().motion({ scale: 0.95 }).style
        : "opacity:0.6";
      return `
