/**
 * Clip Reel — nested score.clip() + std.video.sync
 *
 * Phases drive the overall story; each reel segment is a nested clip with its
 * own local score and a frame-synced embedded video.
 */
...
export default defineScene({
  ...
  render(ctx) {
    ...
    const t = std.score({ intro: "1.5s", reel: "6s", outro: "1.5s" });

    const intro = t.clip({ during: "intro" });
    if (intro.active) {
      const local = intro.score({ enter: "40%", hold: "40%", exit: "20%" });
      const title = local.motion({ y: 28, scale: 0.94 });
      const subtitle = local.motion({ y: 16, at: 0.2 });
      return `
    ...
    if (reel.active) {
      ...
      if (active) {
        ...
        const local = clip.score({ enter: "20%", hold: "60%", exit: "20%" });
        ...
        const badge = local.motion({ scale: 0.9, at: 0.15 });
    ...
    if (outro.active) {
      const local = outro.score();
      const cta = local.motion({ y: 24 });
      return `
        <div style="${std.css({ width, height }, std.css.center(), std.css.column())}">
          <span style="${std.css({ fontSize: 52, fontWeight: 700, color: data.accent }, cta.style)}">score.clip + video.sync</span>
