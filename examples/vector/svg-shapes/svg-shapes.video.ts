...
export default defineScene({
  ...
  render(ctx) {
    ...
    const t = std.score({ enter: "1.5s", hold: "2.0s", exit: "1.5s" });
    const opacity = t.motion({ y: 0 }).opacity;

    // Center polygon — slowly rotates, sides animate
    const centerR = std.spring(0, 120, t.within("enter"), { stiffness: 80, damping: 14 });
    ...
    const ringR = 280;
    const items = std.stagger(ringShapes, t.within("enter"), {
      duration: 0.4,
      from: "center",
    });
    ...
    const waveAmp = std.interpolate(t.within("enter"), [0, 1], [0, 50], "easeOutCubic");
