...
export default define({
  ...
  render(ctx) {
    ...
    const segIdx = Math.min(Math.floor(timeline.progress * 3), 2);
    const segProgress = (timeline.progress * 3) - segIdx;
    ...
    // Overall fade
    const masterOpacity = std.interpolate(timeline.progress, [0, 0.05, 0.95, 1], [0, 1, 1, 0]);
