...
console.log(result); // 5`,
  ...
  render(ctx) {
    ...
    // Director phases: 1s enter | 3s hold | 1s exit
    const t = ctx.director({ enter: "1.0s", hold: "3.0s", exit: "1.0s" });

    // Static code animation
    const staticAnim = t.motion({ at: "0%", for: "80%", y: 30 });

    // Dynamic code animation (staggered +0.3 in the 1s enter phase = 0.3s)
    const dynamicAnim = t.motion({ at: "30%", for: "80%", y: 30 });
