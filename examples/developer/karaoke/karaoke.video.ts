...
export default defineScene({
  ...
  render(ctx) {
    ...
    const scene = std.score({ enter: "0.6s", captions: "2.8s", exit: "0.6s" });
    ...
    // Convert from ElevenLabs format and create transcript sync.
    const words = std.cue.fromElevenLabs(rawTranscript);
