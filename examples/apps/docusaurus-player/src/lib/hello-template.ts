...
export interface HelloData extends Record<string, unknown> {
  title: string;
  subtitle: string;
  accentColor: string;
}

export const helloTemplate = define<HelloData>({
  sample: {
    title: "Hello, SuperImg!",
    subtitle: "Video previews in Docusaurus",
    accentColor: "#667eea",
  },
  config: {
    fps: 30,
    ...
    return `
      <div style="${std.css({ width, height, position: "relative", overflow: "hidden" })}">
        <div style="position:absolute; inset:0;
          background:
            radial-gradient(ellipse 700px 500px at 20% 30%, ${std.color.alpha(data.accentColor, 0.22)}, transparent 70%),
        ...
        <div style="${std.css(std.css.center())}">
          <div style="text-align:center; padding:0 80px; ${title.style}">
            <div style="width:${lineW}px; height:3px; margin:0 auto 24px; border-radius:2px;
              background:linear-gradient(90deg, transparent, ${data.accentColor}, transparent);"></div>
  ...
  render(ctx) {
    ...
    return `
      <div style="${std.css(
        ...
        Gradient ${Math.floor(timeline.progress * 100)}%
      </div>
    `;
  },
});