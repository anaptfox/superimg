...
export default defineScene({
  ...
  render(ctx) {
    const { std, width, height, data, sceneTimeSeconds, sceneProgress, isPortrait } = ctx;
    ...
    const activeChunkIndex = Math.max(
      ...
      chunks.findIndex((chunk, index) => {
        ...
        return sceneTimeSeconds >= chunk.start && sceneTimeSeconds < (next?.start ?? chunk.end + 0.35);
      }),
    );
    ...
    const chunkProgress = activeChunk
      ? std.math.clamp((sceneTimeSeconds - activeChunk.start) / Math.max(0.01, activeChunk.end - activeChunk.start), 0, 1)
      : 0;
    ...
    const intro = std.interpolate(sceneProgress, [0, 0.08, 0.92, 1], [0, 1, 1, 0], "easeOutCubic");
    const progressPct = chunks.length > 1 ? safeChunkIndex / (chunks.length - 1) : sceneProgress;
    const sourceLabel = (() => {
      try {
