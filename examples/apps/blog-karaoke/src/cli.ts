...
import { buildKaraokeData } from "./timing.js";
...
function printHelp() {
  console.log(`
  blog-karaoke — turn a blog post into a karaoke read-along video

  Usage:
    blog-karaoke <url> [options]
    blog-karaoke --text ./post.txt [options]
...
async function main() {
  ...
  const article = await loadArticle({ url: args.url, textPath: args.textPath ? resolve(args.textPath) : undefined });
  ...
  const output = args.output ?? resolve(outputDir, "blog-karaoke.mp4");
